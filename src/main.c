...
static const char *boardID = "home5001";
...
void app_main() {
    ...
    */
    ...
    esp_log_level_set("WIFI01", ESP_LOG_WARN);
    ...
    // timezone setup --  TODO: Choose from screen/parameters
    ...
    ESP_LOGI(TAG, "ESP_ID: room_tempVal:  %s-------------------", room_tempVal);

    strcpy(room_tempSp, "/");
    strcat(room_tempSp, boardID);
    strncat(room_tempSp, "/temp/spvalue", 30);
    ESP_LOGI(TAG, "ESP_ID: room_tempVal:  %s-------------------", room_tempSp);

    strcpy(heater_cmd, "/");
    strcat(heater_cmd, boardID);
    strncat(heater_cmd, "/heater/command", 30);
    ESP_LOGI(TAG, "ESP_ID: heater_cmd:  %s-------------------", heater_cmd);    

    strcpy(heater_stt, "/");
    strcat(heater_stt, boardID);
    strncat(heater_stt, "/heater/status", 30);
    ESP_LOGI(TAG, "ESP_ID: heater_status:  %s-------------------", heater_stt);
    ...
    for (;;) {
        ...
        case 1: // over db => SET HEATER OFF
            ...
            if (heater_command.value_actual != heater_command.value_prev) {
                // gpio_set_level(GPIO_OUTPUT_01, OFF);                                        // GPIO_01 = BOILER, TODO
                ESP_LOGI(TAG, "%d / %d:%d SET HEATER OFF. Setpoint: %3.2f, Temperature: %3.2f ",
                ...
                if (mqtt_connected() == ESP_OK) {
                    ...
                    msg_id = mqtt_client_publish(heater_cmd, 0, 0, 0, 0);
                }
            }
            break;
        ...
        case 3: // under db => SET HEATER ON
            ...
            if (heater_command.value_actual != heater_command.value_prev) {
                // gpio_set_level(GPIO_OUTPUT_01, ON);                                        // GPIO_01 = BOILER, TODO
                ESP_LOGI(TAG, "%d / %d:%d SET HEATER ON. Setpoint: %3.2f, Temperature: %3.2f ",
                         timeinfo.tm_wday, timeinfo.tm_hour, timeinfo.tm_min,
                         temperature_setpoint.value, BMP280_Measures.temperature.value);
                if (mqtt_connected() == ESP_OK) {
                    //msg_id = mqtt_client_publish(heater_cmd, "ON", 0, 0, 0);
                    msg_id = mqtt_client_publish(heater_cmd, 1, 0, 0, 0);
                }
            }
            break;
        ...
        if ((timeinfo.tm_min % 5) == 0) {
            ...
            // MQTT Publish
            if (mqtt_connected() == ESP_OK) {
                ESP_LOGI(TAG, "Write values to MQTT broker.");   
                ...
                //if (heater_command.value_actual == 0) { strcpy(valor, "OFF"); }
                //else { strcpy(valor, "ON"); };
                if (heater_command.value_actual == 0) { strcpy(valor, "0"); }
                else { strcpy(valor, "1"); };
                msg_id = mqtt_client_publish(heater_stt, valor, 0, 0, 0);
            }
            else { ESP_LOGI(TAG, "MQTT messaging not active"); 
            }
        }
    }
}
